 ...
 */
export function createSearchOptimizer<T = any>(
  executor: SearchExecutor<T>,
  options: SearchOptimizerOptions<T> = {},
): SearchOptimizerResult<T> {
  ...
  const {
    ...
    inputProcessor,
    onSearchStart: initialOnSearchStart,
    onSearchSuccess: initialOnSearchSuccess,
    onSearchError: initialOnSearchError,
    onSearchCanceled: initialOnSearchCanceled,
  } = options;

  // Internal state
  let currentQuery = '';
  let previousQuery = '';
  let searchResults: T | null = null;
  ...
  const executeSearch = async (query: string): Promise<void> => {
    ...
    try {
      // Execute the search
      const results = await executor.execute(query, requestController?.signal);
  ...
  const instance: SearchOptimizerResult<T> = {
    get query() {
      return currentQuery;
    },
    get loading() {
